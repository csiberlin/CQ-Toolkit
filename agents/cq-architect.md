...
**A `clean` verdict must be earned — `clean` means *inspected and clean*, never *not looked at*.** The allowed verdicts are `clean` / `<N> findings` / `partial — sampled <N> of <M>` / `not fully assessed` (/ `not applicable` where noted). Every row carries a **Basis** cell naming what you actually inspected to reach the verdict. For `clean`, the basis must cite the concrete surfaces checked — not "looked fine." **If coverage was sampled, say so in the verdict — never a bare `clean` over the unsampled remainder:** use `partial — sampled <N> of <M>` and name the skipped slice in the Basis (e.g. `sampled 4 of 11 backend projects; messaging + scheduler tiers not traced`). A bare `clean` asserts the *whole* dimension is fine; only claim it when the whole dimension was inspected. A dimension you did not inspect deeply enough to defend a `clean` (or an honest `partial`) basis MUST be marked **`not fully assessed`**, never `clean`: an unearned green stamp is *worse* than a missing finding, because it tells the reader "checked, nothing here" and actively suppresses follow-up. If you cannot fill the basis line, you cannot claim `clean`.
