...
3. **Cross-lens reconciliation** (Step 4b below) — diff the per-unit `## Cross-Lens Flags` against the owners' actual findings, reconcile contradictory severities across lenses, and cross-check each lens's coverage-map `clean` verdicts against the union of all lenses' findings for that dimension. This is the only step that sees all lenses for a unit at once, so it is the toolkit's safety net against the three failure modes that motivated this pass: a flagged issue that no lens ends up owning, one lens saying "no action" on a path another rates Medium+, and one lens stamping a dimension `clean` while another reports a real issue inside it.
...
**(c) Coverage-map `clean` stamped over another lens's finding.** Each per-unit report carries a `## Coverage map` whose rows now read `<verdict> (+ one-line basis)`, with verdicts `clean` / `<N> findings` / `not fully assessed`. `Read` those maps from the per-unit reports (a sanctioned Phase-B dip). For every dimension a lens stamped **`clean`**, cross-check it against the **union of all lenses' `## Findings` and `## Cross-Lens Flags` for the same unit and dimension**. If any lens reported a real issue in a dimension another lens (or its own map) called `clean`, the `clean` is false confidence: surface the contradiction in `## Verification log`, and carry the issue forward at the reporting lens's severity (it does not vanish because a different lens stamped green). The canonical case: an Architect `Observability | clean` verdict over a CodeReview finding that a deployed `appsettings.{Env}.json` silences the app's own log trace — observability is not clean, CodeReview owns the finding, and the Architect's green stamp must not bury it. A `not fully assessed` verdict is an explicit coverage gap, not a clean — record it in `## Per-solution gaps` with the dimension and the lens that under-covered it; never read it as `clean`.
...
- **Cross-check coverage-map `clean` verdicts against all lenses' findings** (Step 4b (c)): for every dimension a lens stamped `clean`, confirm no other lens (or that lens's own findings) reported an issue in the same dimension for the same unit. A `clean` contradicted by a real finding is false confidence — surface it in `## Verification log` and carry the issue forward at the reporting lens's severity. Treat a `not fully assessed` verdict as a coverage gap (`## Per-solution gaps`), never as `clean`.
