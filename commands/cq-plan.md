...
- For each `.sln`, compute the expected purpose file path:
...
- For every `.sln` whose expected purpose file is **missing**, dispatch a `CQ-Business-Value` agent (single message, multiple parallel `Agent` calls — batch in groups of 6–10 if there are many).
  - `subagent_type: "CQ-Business-Value"`
  - Brief each agent with: absolute path of the `.sln`, that the working directory is the current shell cwd, and that the report MUST land at `<cwd>\CQ-Reviews\solutions\<Solution-Name>\Purpose.md` (the agent already follows this convention — just confirm it).
